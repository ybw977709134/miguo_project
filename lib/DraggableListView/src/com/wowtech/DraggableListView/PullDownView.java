...
public class PullDownView extends LinearLayout implements ScrollOverListView.OnScrollOverListener {
    ...
    private static final int DEFAULT_HEADER_VIEW_HEIGHT = 80;  // 头部文件原本的高度
    ...
    private boolean loadMoreFinishScrollToBottom=false;
...
//    private boolean mIsPullUpDone;      // 是否回推完成
    ...
    private String lastRefreshTimeFormat;

    private int lvLastItemIndex;
    private int lvTopOffset;
...
//            mFooterLoadingView.setVisibility(View.VISIBLE);
    ...
    private void initHeaderViewAndFooterViewAndListView(Context context){
        lastRefreshTimeFormat=context.getString(R.string.last_refresh_time);
        ...
        mFooterView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if(!mIsFetchMoreing){
                    ...
                    mFooterTextView.setText(R.string.loading_more_in_progress);
                    mFooterLoadingView.setVisibility(View.VISIBLE);
        ...
        mListView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScroll(AbsListView absListView, int i, int i2, int i3) {
                lvLastItemIndex=mListView.getFirstVisiblePosition();
                View v = mListView.getChildAt(0);
                lvTopOffset = (v == null) ? 0 : v.getTop();
            }
        });
    ...
    private void checkHeaderViewState(){
        if(mIsRefreshing) {
            mHeaderTextView.setText(R.string.loading_more_in_progress);
...
//                    mHeaderViewDateView.setVisibility(View.VISIBLE);
                    mHeaderViewDateView.setText(String.format(lastRefreshTimeFormat,dateFormat.format(new Date(System.currentTimeMillis()))));
                ...
                case WHAT_DID_MORE :
                    ...
                    mFooterTextView.setText(R.string.load_more);
                    mFooterLoadingView.setVisibility(View.GONE);
                    if(loadMoreFinishScrollToBottom) {
                        mListView.setSelection(mListView.getAdapter().getCount()-2);
                    }
                    showFooterView();
                    break;
            }
    ...
    private void restoreListViewPosition() {
        mListView.setSelectionFromTop(lvLastItemIndex, lvTopOffset);
    }
    ...
    private void showFooterView(){
        if(mListView.getFooterViewsCount() == 0 && isFillScreenItem()) {
            ...
            mListView.setAdapter(mListView.getContentAdapter());
        }
    ...
    public boolean onListViewTopAndPullDown(int delta) {
        //when refreshing or listView empty,show "loading_in_progress" and draggable down
//        if(mIsRefreshing || mListView.getCount() - mListView.getFooterViewsCount() == 0) return false;
    ...
    public boolean onListViewBottomAndPullUp(int delta) {
        ...
        if(isFillScreenItem()){
            ...
            mFooterTextView.setText(R.string.loading_more_in_progress);
            mFooterLoadingView.setVisibility(View.VISIBLE);
            loadMoreFinishScrollToBottom=true;
            mOnPullDownListener.onMore();
            return true;
        }
        return false;
    ...
    public boolean onMotionDown(MotionEvent ev) {
        loadMoreFinishScrollToBottom=false;
//        mIsDown = true;
//        mIsPullUpDone = false;
...
//                mIsPullUpDone = true;
    ...
    public void triggerRefresh(int delta) {
        setHeaderHeight(0);
        if(!mIsRefreshing && delta>=0){
            mIsRefreshing = true;
            mUIHandler.sendEmptyMessage(WHAT_ON_REFRESH);
        }
    }
