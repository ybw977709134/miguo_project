...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    /**
     * If this value is non-zero, it will be used instead of header's height
     * in {@link #pullEvent()} to determine whether the mode can switch from
     * PULL_TO_REFRESH to RELEASE_TO_REFRESH.
     */
    private int mHeaderPullThreshold = 0;

    protected int mRefreshableViewMarginTop = 0;
    ...
    private boolean mRefreshableViewWrapperHeightAdjusted = false;
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					if (isRefreshing()) {
						smoothScrollTo(-mRefreshableViewMarginTop / 2);
						return true;
					}
		...
		if (doScroll) {
			...
			if (mShowViewWhileRefreshing) {
				...
				switch (mCurrentMode) {
					...
					case PULL_FROM_START:
                        smoothScrollTo(-mRefreshableViewMarginTop / 2
                                -(mHeaderPullThreshold > 0 ? mHeaderPullThreshold : getHeaderSize()),
                                listener);
						break;
				}
			} else {
				smoothScrollTo(-mRefreshableViewMarginTop / 2, listener );
			}
		} else {
	...
	protected final void refreshRefreshableViewSize(int width, int height) {
		...
		switch (getPullToRefreshScrollDirection()) {
			...
			case VERTICAL:
				if (lp.height != height) {
					lp.height = height - mRefreshableViewMarginTop; // see also setupRefreshableViewWrapperHeight()
					mRefreshableViewWrapper.requestLayout();
				}
				break;
		}
	...
	private void init(Context context, AttributeSet attrs) {
        ...
        // setup mRefreshableViewMarginTop
        if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewMarginTop)) {
            mRefreshableViewMarginTop = (int)a.getDimensionPixelOffset(R.styleable.PullToRefresh_ptrRefreshableViewMarginTop, 0);
            if (mRefreshableViewMarginTop != 0) {
                mRefreshableViewWrapper.getViewTreeObserver().addOnGlobalLayoutListener(
                        new ViewTreeObserver.OnGlobalLayoutListener() {
                            @Override
                            public void onGlobalLayout() {
                                if (mRefreshableViewWrapperHeightAdjusted)
                                    return;
                                setupRefreshableViewWrapperHeight();
                                mRefreshableViewWrapperHeightAdjusted = true;
                            }
                        }
                );
            }
    ...
    private void setupRefreshableViewWrapperHeight() {
        LayoutParams lp = (LayoutParams) mRefreshableViewWrapper.getLayoutParams();
        if (lp != null) {
            lp.height -= mRefreshableViewMarginTop;
            mRefreshableViewWrapper.requestLayout();
        }
    }
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			...
			default:
				...
				itemDimension = mHeaderPullThreshold > 0 ? mHeaderPullThreshold : getHeaderSize();
				break;
		}

		setHeaderScroll(newScrollValue - mRefreshableViewMarginTop / 2 );
		...
		if (newScrollValue != 0 && !isRefreshing()) {
            ...
            if (mOnPullEventListener != null) {
                mOnPullEventListener.onPullScaleChanged(scale);
            }
    ...
    public void setRefreshableViewMarginTop(int value) {
        if (mRefreshableViewMarginTop != value) {
            mRefreshableViewMarginTop = value;
            setupRefreshableViewWrapperHeight();
            smoothScrollTo(-mRefreshableViewMarginTop / 2);
        }
    }
    ...
    public static enum AnimationStyle {
		 ...
		 * arrow bitmapfun which flips depending on where the user has scrolled.
		 */
		FLIP,
    ...
    /**
     * If this value is larger than 0, it will be used instead of header's height
     * in pulling progress to determine whether the mode can switch from
     * PULL_TO_REFRESH to RELEASE_TO_REFRESH.
     *
     * @param value
     */
    public void setHeaderPullThreshold(int value) {
        mHeaderPullThreshold = value;
    }

    public int getHeaderPullThreshold() {
        return mHeaderPullThreshold;
    }
}
