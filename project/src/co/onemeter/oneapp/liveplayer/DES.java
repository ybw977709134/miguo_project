	...
	public String authcode(String content, String operation, String key) {
		...
		if (operation != null && operation.equals("DECODE")) {
			if (!TextUtils.isEmpty(content) && !TextUtils.isEmpty(key)) {
				...
				encontent = new String(android.util.Base64.encode(encontent.getBytes(), android.util.Base64.DEFAULT));
			}
		} else if (operation != null && operation.equals("ENCODE")) {
			if (!TextUtils.isEmpty(content) && !TextUtils.isEmpty(key)) {
				encontent = new String(android.util.Base64.decode(content.getBytes(), android.util.Base64.DEFAULT));
				encontent = decrypt(encontent, key);
			}
		}
		return encontent;
	}
